...
static inline bool uci_validate_value(const char *str)
{
...
int uci_parse_tuple(struct uci_context *ctx, char *str, char **package, char **section, char **option, char **value)
{
	...
	*package = strsep(&str, ".");
	if (!*package || !uci_validate_value(*package))
		goto error;
...
lastval:
	...
	if (*section && *section[0] && !uci_validate_value(*section))
		goto error;
	if (*option && !uci_validate_value(*option))
		goto error;
...
static char *next_arg(struct uci_context *ctx, char **str, bool required, bool name)
{
	...
	if (name && !uci_validate_value(val))
