...
/**
 * load_merge_uci: Parse an uci config file and store it in the uci context
 *
 * @ctx: uci context
 * @name: name of the config file (relative to the config directory)
 * @package: store the loaded config package in this variable
 */
extern int load_merge_uci(struct uci_context *ctx, const char *name, struct uci_package **package);
...
extern int uci_check_change(struct uci_context *ctx, struct uci_package *p, char *section, char *option, char *value, struct uci_element **result);
...
extern int uci_set_new(struct uci_context *ctx, struct uci_package *p, char *section, char *option, char *value, struct uci_element **result);
 ...
 *
 * The default backend is "file", which uses /usr/config for config storage
 */
...
struct uci_backend
{
	...
	struct uci_package *(*load_merge)(struct uci_context *ctx, const char *name);
