...
static const char *uci_basedir = UCI_BASEDIR;
...
int load_merge_uci(struct uci_context *ctx, const char *name, struct uci_package **package)
{
	struct uci_package *p;
	UCI_HANDLE_ERR(ctx);
	UCI_ASSERT(ctx, ctx->backend && ctx->backend->load);
	p = ctx->backend->load_merge(ctx, name);
	if (package)
		*package = p;
...
void uci_free_content(struct uci_context *ctx)
{
	struct uci_element *e, *tmp;
...
int uci_commit(struct uci_context *ctx, struct uci_package **package, bool overwrite)
{
	...
	//add by james.xu for send trap to server
	//system("/etc/snmp/snmp_send_trap  config_alter 2>/dev/null &");
	//end
	return 0;
}
