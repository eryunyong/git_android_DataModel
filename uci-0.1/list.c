...
int uci_lookup(struct uci_context *ctx, struct uci_element **res, struct uci_package *p, char *section, char *option)
{
	...
	UCI_ASSERT(ctx, section && uci_validate_value(section));
	if (option)
		UCI_ASSERT(ctx, uci_validate_value(option));
...
int uci_check_change(struct uci_context *ctx, struct uci_package *p, char *section, char *option, char *value, struct uci_element **result)
{
	/* NB: UCI_INTERNAL use means without history tracking */
	...
	UCI_HANDLE_ERR(ctx);
	UCI_ASSERT(ctx, p != NULL);
	UCI_ASSERT(ctx, uci_validate_value(section));
	if (option)
	{
		UCI_ASSERT(ctx, uci_validate_value(option));
		UCI_ASSERT(ctx, value != NULL);
	...
	s = uci_to_section(e);
        if (!option)
	{
	    if(s && s->type &&  value && strcmp(s->type, value) == 0)
	    {
		return 0;
	    }
	}
	...
	if (option)
	{
		...
		if (!e)
            return -1;
		o = uci_to_option(e);
		if(o && o->value&& value && strcmp(o->value, value) == 0)
		{
			return 0;
		}
	}
...
//在显示的时候，不需要增量的概念
int uci_set_new(struct uci_context *ctx, struct uci_package *p, char *section, char *option, char *value, struct uci_element **result)
{
	/* NB: UCI_INTERNAL use means without history tracking */
	bool internal = ctx->internal;
	struct uci_element *e = NULL;
